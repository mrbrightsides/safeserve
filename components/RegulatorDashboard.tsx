...
import { 
  ...
  Trash2, PieChart as PieChartIcon, Utensils
} from 'lucide-react';
...
const mockAggregatedWaste = [
  { type: 'Grains', amount: 420, region: 'Jakarta Raya', vendorType: 'UMKM' },
  { type: 'Vegetables', amount: 380, region: 'Jawa Barat', vendorType: 'Industrial' },
  { type: 'Proteins', amount: 250, region: 'Jawa Timur', vendorType: 'UMKM' },
  { type: 'Fruits', amount: 180, region: 'Sumatera Utara', vendorType: 'Industrial' },
  { type: 'Dairy', amount: 120, region: 'Sulawesi Selatan', vendorType: 'UMKM' },
];
...
const RegulatorDashboard: React.FC = () => {
  ...
  // Waste Filtering States
  const [wasteRegionFilter, setWasteRegionFilter] = useState('ALL');
  const [wasteVendorTypeFilter, setWasteVendorTypeFilter] = useState('ALL');

  const filteredWasteData = useMemo(() => {
    return mockAggregatedWaste.filter(item => {
      const matchesRegion = wasteRegionFilter === 'ALL' || item.region === wasteRegionFilter;
      const matchesType = wasteVendorTypeFilter === 'ALL' || item.vendorType === wasteVendorTypeFilter;
      return matchesRegion && matchesType;
    });
  }, [wasteRegionFilter, wasteVendorTypeFilter]);
    ...
    <div className="space-y-6 pb-20 max-w-7xl mx-auto text-left animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 animate-in fade-in duration-500">
                ...
                <div className="md:col-span-7 flex flex-col justify-between">
                  <div className="space-y-6">
                     ...
                     {/* AI Waste Insights Subsection */}
                     <div className="mt-6 p-6 bg-indigo-50 border border-indigo-100 rounded-[2rem]">
                       <div className="flex items-center gap-2 mb-4">
                         <Trash2 className="w-4 h-4 text-indigo-600" />
                         <h4 className="text-[10px] font-black text-indigo-900 uppercase tracking-widest">AI Waste Pattern Insights</h4>
                       </div>
                       <div className="space-y-3">
                         <div className="flex items-start gap-3 p-3 bg-white/60 rounded-xl border border-indigo-100/50">
                           ...
                           <p className="text-[11px] font-bold text-indigo-800">
                             Vendor frequently wastes <span className="font-black">bread items on Tuesdays</span>. Consider adjusting order quantities for early-week cycles.
                           </p>
                         </div>
                         <div className="flex items-start gap-3 p-3 bg-white/60 rounded-xl border border-indigo-100/50">
                           <TrendingDown className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                           <p className="text-[11px] font-bold text-indigo-800">
                             Vegetable waste spike detected during <span className="font-black">rainy days</span>. Recommend implementing dynamic portioning based on school attendance forecasts.
                           </p>
                         </div>
                       </div>
                     </div>
          ...
          {/* Aggregated Food Waste Visualization */}
          <div className="bg-white p-8 rounded-[2.5rem] border border-gray-200 shadow-sm">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
              <div className="flex items-center gap-3">
                <div className="bg-emerald-50 p-2.5 rounded-2xl shadow-sm"><Trash2 className="text-emerald-600 w-5 h-5" /></div>
                <div>
                  <h3 className="text-xl font-black text-gray-900 tracking-tight">Aggregated Food Waste</h3>
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Waste Volume by Item Type (Kg)</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <select 
                  value={wasteRegionFilter}
                  onChange={(e) => setWasteRegionFilter(e.target.value)}
                  className="px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-indigo-100"
                >
                ...
                <select 
                  value={wasteVendorTypeFilter}
                  onChange={(e) => setWasteVendorTypeFilter(e.target.value)}
                  className="px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-indigo-100"
                >
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={filteredWasteData}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  <XAxis dataKey="type" axisLine={false} tickLine={false} tick={{fontSize: 10, fontWeight: 'bold', fill: '#94a3b8'}} />
                  <YAxis axisLine={false} tickLine={false} tick={{fontSize: 10, fontWeight: 'bold', fill: '#94a3b8'}} />
                  <RechartsTooltip cursor={{fill: 'transparent'}} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontSize: '10px' }} />
                  <Bar dataKey="amount" fill="#10b981" radius={[8, 8, 0, 0]}>
                    {filteredWasteData.map((entry, index) => (
